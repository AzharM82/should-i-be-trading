        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="text-t-muted border-b border-t-border">
              ...
              <th className="text-left py-1.5 pl-3 font-medium w-1/3">Bar</th>
            </tr>
          </thead>
          <tbody>
            {categories.map((cat) => {
              ...
              let barColor: string;
              if (cat.score >= 65) barColor = "bg-t-green/50";
              else if (cat.score >= 40) barColor = "bg-t-amber/50";
              else barColor = "bg-t-red/50";

              return (
                <tr key={cat.name} className="border-b border-t-border/50">
                  <td className="py-2 text-t-text">{cat.name}</td>
                  <td className="py-2 text-right text-t-muted">{(cat.weight * 100).toFixed(0)}%</td>
                  <td className="py-2 text-right font-medium text-t-text">{cat.score}</td>
                  <td className="py-2 text-right text-t-muted">{contribution}</td>
                  <td className="py-2 pl-3">
                    <div className="h-3 bg-t-bg rounded overflow-hidden">
                      <div
          ...
          <tfoot>
            <tr className="font-medium">
              <td className="py-2 text-t-blue">TOTAL</td>
              <td className="py-2 text-right text-t-muted">100%</td>
              <td className="py-2 text-right text-t-blue text-sm">{data.qualityScore}</td>
              <td className="py-2 text-right text-t-blue">{data.qualityScore}</td>
              <td className="py-2 pl-3">
                <div className="h-3 bg-t-bg rounded overflow-hidden">
                  <div
                    className="h-full rounded bg-t-blue/50 transition-all duration-500"
