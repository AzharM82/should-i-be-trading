...
export function AlertBanner({ fomc }: Props) {
  ...
  if (fomc.isToday) {
    ...
    severity = "bg-t-red/20 border-t-red/50 text-t-red";
  } else if (fomc.daysUntil === 1) {
    message = `FOMC MEETING TOMORROW (${fomc.nextDate}) — POSITION ACCORDINGLY`;
    severity = "bg-t-amber/20 border-t-amber/50 text-t-amber";
  } else {
    message = `FOMC MEETING IN ${fomc.daysUntil} DAYS (${fomc.nextDate})`;
    severity = "bg-t-amber/10 border-t-amber/30 text-t-amber";
  }

  return (
    <div className={`${severity} border rounded px-4 py-2 mx-3 mt-2 text-xs font-medium text-center tracking-wider`}>
      {message}
    </div>
  );
}
